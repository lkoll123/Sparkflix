...
function WhosWatching() {
    ...
    const handleAddProfile = async (newProfile) => {
        await axios.post('${import.meta.env.VITE_BACKEND_URL}/api/setUser', { ...newProfile, userId: userId })
        .then((res) => {
                console.log(res);
        })
    ...
    const handleRemoveProfile = (profileId) => {
        ...
        axios.post('${import.meta.env.VITE_BACKEND_URL}/api/removeUser', {userId: profileId})
        .then((res) => {
            console.log(res);
        })
        .catch(err => console.log(err));
